...
| **microReticulum** | [attermann/microReticulum](https://github.com/attermann/microReticulum) | [ScotMesh/microReticulum](https://github.com/ScotMesh/microReticulum), pinned by commit in `platformio.ini`. It carries the stack fixes the P1 needed: neighbour probes no longer freeze the node inside `Transport::jobs()`, and full path-table walks feed the watchdog instead of restarting the node. Its README lists every difference from upstream. |
